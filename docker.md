...
## 5. Redis (รันบน Docker)

Service ใช้ Redis เก็บ **refresh token + shadow password hash** (ดู auth flow)
**ต้องเป็น Redis ≥ 6.2** — go-redis v9 ส่ง `SET key val EX/PX <ttl>` ซึ่งมีตั้งแต่ Redis 2.6.12

> ⚠️ **เหตุที่ต้องย้ายมา Docker:** เครื่อง dev เคยมี **Redis 2.4.6** (Windows service port เก่า ~2012)
> รันเป็น service ชื่อ `redis` ตัวนั้น `SET ... EX/PX` ไม่รองรับ → `refresh.Save` พัง →
> **login ตอบ HTTP 500** (`INTERNAL_ERROR`) ทั้งที่เบอร์/รหัสถูก — read (`GET`/ping) ผ่านเลยหลอกตา

### 5.1 ลบ Redis เก่าบน Windows ก่อน (สำคัญ — ทำครั้งเดียว)

`redis-server` เก่ายึด `0.0.0.0:6379` อยู่ ต้องหยุด ไม่งั้น container bind 6379 ไม่ได้ + ถ้าปล่อยไว้
จะมี **Redis 2 ตัวแย่ง 6379** (เก่าฟัง IPv4 `127.0.0.1`, docker ฟัง IPv6 `::1`) → client สุ่มโดนตัวไหนก็ได้ → login พังๆ หายๆ

เปิด **PowerShell แบบ Run as Administrator** แล้ว:
```powershell
Stop-Service redis -Force
Set-Service redis -StartupType Disabled    # กันมันกลับมาตอนรีบูต
# sc.exe delete redis                       # (ออปชัน) ลบ service ทิ้งถาวร
Get-NetTCPConnection -LocalPort 6379 -State Listen -ErrorAction SilentlyContinue  # ว่าง = ไม่มีอะไรขึ้น
```

### 5.2 เปิด Redis — 2 ทาง (เลือกอย่างใดอย่างหนึ่ง)

**ทาง A — compose** (เพิ่ม service `redis` ในไฟล์เดิมแล้ว):
```powershell
docker compose up -d redis      # สตาร์ทเฉพาะ redis (ไม่แตะ api — api ยังรันด้วย air)
```

**ทาง B — docker run standalone:**
```powershell
docker run -d --name go-api-redis -p 6379:6379 -v redis-data:/data --restart unless-stopped redis:7-alpine redis-server --appendonly yes
```

> ทั้งสองทางใช้ชื่อ/พอร์ตเดียวกัน รันซ้อนกันชน — สลับทางต้อง `docker rm -f go-api-redis` (หรือ `docker compose down`) ก่อน

### 5.3 ตรวจ + ทดสอบ
```powershell
docker exec go-api-redis redis-cli INFO server | Select-String redis_version   # ต้อง 7.x
docker exec go-api-redis redis-cli SET k v EX 10    # 7.x = OK (2.4.6 เคยเด้ง wrong number of arguments)
curl.exe -s -X POST http://localhost:8080/api/v1/auth/login -H "Content-Type: application/json" -d '{\"phone_number\":\"08xxxxxxxx\",\"password\":\"xxxx\"}'
# ต้องได้ {"success":true,"data":{"access_token":...}}  → login ผ่าน
```

### 5.4 หมายเหตุ
- `.env` ใช้ `REDIS_ADDR=localhost:6379` เหมือนเดิม — map `6379:6379` แล้ว `air` (รันโลคอล) ต่อติดทันที ไม่ต้องแก้
- ข้อมูลถาวรอยู่ใน named volume `redis-data` (AOF) — `docker rm` container ไม่หาย; ลบจริงต้อง `docker volume rm redis-data`
- ถ้าจะรัน **api เป็น container** ด้วย (ไม่ใช่ air) ให้ตั้ง `REDIS_ADDR=redis:6379` ใน `.env` (ต่อผ่านชื่อ service ใน compose network)

### 5.5 RedisInsight — GUI ดู/แก้ข้อมูล Redis (optional, dev tool)

ทางการของ Redis ฟรี — ดู key เป็น tree, ดู value/TTL/type, มี CLI + Profiler (GUI ของ `MONITOR`)
เพิ่ม service `redisinsight` ใน `docker-compose.yml` แล้ว (มี volume เก็บ config ถาวร)

```powershell
docker compose up -d redisinsight       # หรือ standalone:
docker run -d --name redisinsight --restart unless-stopped -p 5540:5540 -v redisinsight-data:/data redis/redisinsight:latest
```

เปิด **http://localhost:5540** → กด **`+ Connect existing database`** (ตั้งครั้งเดียว เพราะมี volume)

> ⚠️ **อย่ากด "Create free Cloud database"** — อันนั้นคือสมัคร Redis Cloud บนเน็ต ไม่ใช่ตัวที่รันในเครื่อง

กรอกค่า:

| ช่อง | ผ่าน compose | รัน standalone (`docker run`) |
|---|---|---|
| **Host** | `redis` (ชื่อ service) | `host.docker.internal` |
| **Port** | `6379` (ไม่ใช่ 5540 — นั่นคือพอร์ต UI) | `6379` |
| **Username / Password** | เว้นว่าง | เว้นว่าง |

> RedisInsight อยู่ในคอนเทนเนอร์ → `localhost` ของมันคือตัวมันเอง ต้องชี้ผ่านชื่อ service (compose) หรือ `host.docker.internal` (standalone)

กด **Add Redis Database** → คลิกชื่อ DB ในลิสต์ → เข้าหน้า **Browser** เห็น key ทั้งหมด

**ดูข้อมูลที่ service เก็บ** (เมนู *Browser*): `refresh:<hash>` (string, JSON token), `cust_refresh:<id>` (set, index ไว้ revoke), `pwhash:<id>` (string, shadow argon2 ถาวร) — คลิก key เห็น value/TTL/type แก้/ลบได้
...
| login 500 `INTERNAL_ERROR` (เบอร์/รหัสถูก) | Redis เก่าเกิน (< 2.6.12) — `SET ... EX/PX` ไม่รองรับ → ย้ายมา Redis 7 บน Docker (ข้อ 5) |
| login พังๆ หายๆ / `redis_version` เป็น 2.4.6 บน IPv4 | Windows redis เก่ายังรันแย่ง 6379 กับ docker (IPv4 vs IPv6) → ทำข้อ 5.1 (`Stop-Service redis` + disable) |


# 1. ลบ container เก่า
docker rm -f ws-service

# 2. ลบ image เก่า
docker rmi ws-service

# 3. build ใหม่
docker build -t ws-service .

# 4. run container
docker run --name ws-service \
  --env-file .env \
  -p 8081:80 \
  ws-service

# 5. copy .env เข้า container
docker cp .env ws-service:/home/site/wwwroot/.env